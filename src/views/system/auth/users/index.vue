<template>
  <div>
    <el-row :gutter="20">
      ...
      <el-col :span="20">
        <div class="grid-content">
          ...
          <el-button v-permission="$service.user.permission.add" size="small" type="primary" @click="handleAddUser">新增用户</el-button>
          ...
          >
              ...
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
                  ...
                  <el-form-item label="最后登录时间">
                    <span v-if="props.row.last_login">{{ props.row.last_login |userTime }}</span>
                  </el-form-item>
            ...
            <el-table-column label="机构" prop="areaname" width="180" align="center" />
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                <el-button  size="small" type="primary" icon="el-icon-edit" @click="handleEdit(scope)">编辑</el-button>
                <el-button v-permission="$service.user.permission.delete" size="small" type="danger" icon="el-icon-delete" @click="handleDelete(scope)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog :title="dialogType==='edit' ? '用户编辑':'新增用户' " :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form v-if="dialogVisible" ref="userForm" :model="user" :rules="userRules" status-icon size="medium" label-width="80px" label-position="left">
        ...
        <el-form-item label="账号" prop="username">
          <el-input v-model.trim="user.username" placeholder="请输入账号" clearable :disabled="dialogType!=='new'" />
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model.trim="user.password" type="password" clearable placeholder="请输入密码" />
        </el-form-item>
        <el-form-item label="密码" prop="checkPass">
          <el-input v-model.trim="user.checkPass" type="password" clearable placeholder="请再次输入密码" />
        </el-form-item>
        <el-form-item label="手机" prop="phone">
          <el-input ref="phone" v-model.trim="user.phone" clearable placeholder="请输入手机号" />
        </el-form-item>
...
const defaultUser = {
    ...
    phone: '',
    nick_name: '',
    avatar: 'https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=3866513739,3961195204&fm=26&gp=0.jpg',
    remark: '',
    roleList: []
}
export default {
    ...
    data() {
        ...
        return {
            ...
            userRules: {
                area_id: [{ required: true, message: '请选择所属区域', trigger: 'change', type: 'number' }],
                username: [
    ...
    methods: {
        ...
        async handleDelete(scope) { // 调出删除角色确认弹框
            try {
                ...
                console.log('删除', data, scope.$index)
        ...
        async confirmUser() { // 弹出框，用户保存按钮,修改或者新增
            const isEdit = this.dialogType === 'edit'
            if (this.user.password) { // 密码MD5加密处理
                this.user.password = this.$md5(this.$md5(this.user.password).split('').reverse().join(''))
            }
