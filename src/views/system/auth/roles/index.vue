  ...
  <div class="container">
    <el-form :model="query" label-width="80px" inline size="small" ref="form">
      <el-form-item label="部门:">
        <el-cascader v-model="query.areaId" placeholder="请选择部门" :options="deptList" ref="areaTree"
          :props="{
            checkStrictly: true,
            label: 'name',
            value: 'id',
    ...
    <el-table :data="tableData" style="width: 100%; margin-top: 30px" fit size="mini"
      ...
      <el-table-column label="操作" align="center">
        <template v-slot="{ row }">
          <el-button v-permission="'api:v1:role:menu'" size="small" type="text"
            icon="el-icon-setting" @click="handlePerms(row)">权限设置</el-button>
          <el-button v-permission="'api:v1:role:update'" size="small" type="text"
            icon="el-icon-edit" @click="handleEdit(row)">编辑</el-button>
          <el-button v-permission="'api:v1:role:delete'" size="small" type="text"
            icon="el-icon-delete" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <role-dialog :visible.sync="visible" :deptList="deptList" :id.sync="id"
      @success="getList"></role-dialog>
